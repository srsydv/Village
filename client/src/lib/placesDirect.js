...
export async function suggestPlacesDirect(query) {
...
async function geocode(query) {
  const q = String(query || "").trim();
  if (!q) throw new Error("Enter a city or destination.");
  return cached(`geo:${q.toLowerCase()}`, async () => {
    ...
    const rows = await fetchJson(url);
    const hit = rows?.[0];
    if (!hit) throw new Error("That place could not be found. Try a city name.");
    const addr = hit.address || {};
...
export async function lookupDestinationDirect(query) {
