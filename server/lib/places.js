const UA = `SafarTravel/1.0 (${process.env.PUBLIC_APP_URL || "https://aurea-jrvb.onrender.com"}; travel concierge)`;
...
async function nominatimPois(city, origin) {
  ...
  for (const item of searches) {
    try {
      ...
      console.error("Nominatim POI fallback failed", item.kind, err.message);
    }
  }
  return out;
}
